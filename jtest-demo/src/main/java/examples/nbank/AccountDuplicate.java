...
public class AccountDuplicate {
    ...
    public AccountDuplicate(Customer customer, int initial_balance) {
	...
	public boolean isOverdrawnDuplicate(int balance) {         
        if (balance >= 0) {
    ...
    private String setAccountStatus(int balance) {
        ...
        } else if ((balance >= BALANCE_GOLD_MIN) && (balance < BALANCE_PLATINUM_MIN)) { // parasoft-suppress BD.PB.CC "Reviewed, this is intentionally"
            return STATUS_GOLD;
        } else {
